...
void set_vmode(int w, int h) {
    ...
    for(i = 0; modes[i] != 0xFFFF; i++) {
        ...
        //1280 720 32
        if(loc_mib->Xres == w && loc_mib->Yres == h && loc_mib->bpp == 32) {
            break;
        }
    }
...
inline void draw_character_at(int x, int y, int c, uint32_t fg, uint32_t bg) {
